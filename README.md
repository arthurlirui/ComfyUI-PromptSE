...
-   **模型模板加载**: 选择模型后自动加载 `js/templates/*.json` 对应模板，每个条目支持候选词条。
-   **多词条下拉选择 + 自定义保存**: 每个条目可在多选下拉中选择多个词条，也可输入自定义词条并保存回条目候选列表。
-   **LLM输入输出**: 节点会同时输出组合提示词和 `llm_input`（用于直接连接大语言模型节点生成完整提示词）。
